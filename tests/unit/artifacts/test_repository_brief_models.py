...
def test_categorize_unsupported_extension_does_not_error() -> None:
    result = _categorize_paths(["some/file.xyz"])
    assert result.total_files == 1
    assert result.source_files == 0
    assert result.markdown_files == 0
